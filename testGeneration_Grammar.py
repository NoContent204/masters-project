...
UDS_Grammar2: "dict[str,list[str]]" = { # more exhustive version (allows for data to be less than 8 bytes but also more (requiring more than 1 CAN frame to transmit))
    ...
    "<data>": {"options":["<byte><byte>", "<data><byte>"], "probs":[0.04,0.96]},
...
def generateInput():
    ...
    while(len(re.findall(nonTerminalPattern,currentTerm)) !=0): # while there are still non terminal symbols to expand
        ...
        print(expansionList)
        print(probabilites)
        if (len(expansionList) == 1):
            chosenExpansion = expansionList[0]
        else:
